...
Zhang 2018). It imports data on Spanish language learners from SPSS data files 
...
# Import Data
Please see the `README.Rmd` and `README.md` files in this package for details 
on obtaining the SPSS data files referenced by the code below. 

\FloatBarrier

## Student-Level Data (for Correlation Analyses)
The file `df_corr.sav` has the cleaned self-assessment data on a set of Spanish
students, with one row per learner. We archived an SPSS file to simplify 
...
expanded to one row per learner per self-assessment testlet attempted. The data
structure is conceptually similar to a person-period file for longitudinal
analysis. We need the expanded `df_crm` data frame specifically to run
continuation-ratio models. Most other analyses are performed based on the
`df_corr` data frame.
...
### Contrast Coding 
Here we pause to highlight the default contrast coding associated with key
factor variables in `df_crm` so readers know how to interpret model coefficients
later. The default for factors should be dummy coding (treatment contrasts) with
the first level set as the reference group.
